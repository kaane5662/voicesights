...
import { formatDuration, formatRelativeTime } from '@/utils/dates';
const mockChats = [
  {
    id: 1,
...
function ChatListItem({ chat, onSelect, onDelete, sessionId }) {
  ...
  return (
    <div
      className="group relative flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer transition-all"
      onClick={() => onSelect(chat)}
    >
      
      <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 border border-violet-500/30 flex items-center justify-center shrink-0">
        <MessageSquare className="w-3 h-3 text-violet-400" />
      </div>
      
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <h3 className="font-medium text-sm max-w-[90%] text-white truncate">{chat.title}</h3>
          {chat.hasAI && <Sparkles className="w-3.5 h-3.5 text-violet-400 shrink-0" />}
        </div>
        <p className="text-xs text-slate-500 truncate">{formatRelativeTime(chat.created_at)}</p>
      </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        <div className="text-right opacity-100 group-hover:opacity-0 transition-opacity">
          ...
          <p className="text-xs text-slate-600">{chat.messages.length} messages</p>
        </div>
...
export function ChatsList({ onClose, onSelectChat, onNewChat, sessionId }) {
  ...
  const handleDelete = (chatId:string) => {
    if (!window.confirm("Are you sure you want to delete this chat session?")) return false;
    axios.delete(`${SERVER_URL}/chats/${chatId}/`, {withCredentials:true}).then((res)=>{
        // console.log(chats)
        setRecentMessages(prev=>prev.filter(c=>c.id != chatId))
        setOtherMessages(prev=>prev.filter(c=>c.id != chatId))
  ...
  return (
      ...
      <div className="w-[500px] max-w-xl absolute right-0 top-8 bg-slate-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="p-4 border-b border-white/10">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-md font-bold text-white">Chat History</h2>
          ...
          <button
            onClick={onNewChat}
            className="w-full p-2 rounded-xl bg-gradient-to-r from-violet-500/20 to-fuchsia-500/20 border border-violet-500/30 hover:border-violet-400/50 transition-all group"
          >
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 rounded-md bg-gradient-to-br from-violet-500 to-fuchsia-600 flex items-center justify-center shadow-lg shadow-violet-500/25 group-hover:scale-105 transition-transform">
                <Plus className="w-5 h-5 text-white" />
              </div>
              <div className="text-left">
                <h3 className="font-semibold text-sm text-white">Start New Chat</h3>
                <p className="text-xs text-slate-400">Begin a fresh conversation with your transcript</p>
              </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          {recentMessages.length > 0 || otherMessages.length > 0 ? (
            <div className="p-2">
